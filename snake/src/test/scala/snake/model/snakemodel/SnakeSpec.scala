...
class SnakeSpec extends FunSpec with Matchers {
  ...
  describe("Shrinking") {
    ...
    it("should be able to shrink") {
      ...
      s.length shouldEqual 4

      s.shrink.length shouldEqual 3
      s.shrink.start shouldEqual GridPoint(0, 3)
      s.shrink.end shouldEqual GridPoint(0, 1)
      s.shrink.body shouldEqual List(GridPoint(0, 2), GridPoint(0, 1))

      s.shrink.shrink.shrink.start shouldEqual GridPoint(0, 3)
      s.shrink.shrink.shrink.end shouldEqual GridPoint(0, 3)
      s.shrink.shrink.shrink.shrink.shrink.shrink.shrink.length shouldEqual 1
    }
